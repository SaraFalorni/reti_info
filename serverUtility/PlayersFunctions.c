...
int insertPlayer(char* nickname, int client_fd) {
    ...
    if(current_session.players == NULL)
        ...
        struct Player* last_player = findLastPlayer(current_session.players);
...
struct Player* getPlayer(struct Player* current_player,char* nickname) {
    ...
    return getPlayer(current_player->next, nickname);
}
...
struct Player* getPlayerIndex(struct Player* current_player,int index) {
    ...
    return getPlayerIndex(current_player->next, index-1);
}
